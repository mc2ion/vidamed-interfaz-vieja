...
/**
 * Servlet implementation class EditEstimationProtocolAddServlet
 */
@WebServlet(description = "servlet to log in users", urlPatterns = { "/EditEstimationProtocolAddServlet" })
public class EditEstimationProtocolAddServlet extends HttpServlet {
     ...
     */
    public EditEstimationProtocolAddServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userE != null && perm){
			...
			try {
				...
				rd = getServletContext().getRequestDispatcher("/editEstimationProtocolStep2_add.jsp");
			...
			for (int i = 0 ; i < id.length; i++){
				if (id[i] != null){
					...
					try {
						...
						CommandExecutor.getInstance().executeDatabaseCommand(new command.AddEstimationSpecilist(estimationid, protocolid, scaleId, specialistid));
