...
/**
 * Servlet implementation class SearchUnadmittedPatientServlet
 */
@WebServlet(description = "servlet to log in users", urlPatterns = { "/SearchUnadmittedPatientServlet" })
public class SearchUnadmittedPatientServlet extends HttpServlet {
     ...
     */
    public SearchUnadmittedPatientServlet() {
        super();
    }
			...
			if (estimationNumber != null && estimationNumber != ""){
				...
				try {
					...
					try {
						...
						ArrayList<Patient> p = (ArrayList<Patient>) CommandExecutor.getInstance().executeDatabaseCommand(new command.SearchUnadmittedPatient(identityCard, patientType));
					if (p == null || p.size() == 0){
						response.setContentType("text/plain");  
						response.setCharacterEncoding("UTF-8"); 
						response.getWriter().write("not found"); 
