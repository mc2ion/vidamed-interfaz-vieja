...
/**
 * Servlet implementation class ListEcoReportsServlet
 */
@WebServlet(description = "servlet to generate reports", urlPatterns = { "/ListEcoReportsServlet" })
public class ListEcoReportsServlet extends HttpServlet {
		...
		if(userE != null && perm){
			try {
				ArrayList<PatientServiceReport> pp = (ArrayList<PatientServiceReport>) CommandExecutor.getInstance().executeDatabaseCommand(
														new command.GetPatientServiceReport("2"));
				request.setAttribute("pp", pp);
				
				RequestDispatcher rd = getServletContext().getRequestDispatcher("/reports_eco.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userE != null && perm ){
			...
			try {
				...
				@SuppressWarnings("unchecked")
				ArrayList<PatientServiceReport> pp = (ArrayList<PatientServiceReport>) CommandExecutor.getInstance().executeDatabaseCommand(
										new command.GetPatientServiceReport("2", adminId, estimationId, identityCard, gender, type, serviceId, dateIni, dateEnd));
				request.setAttribute("pp", pp);
				...
				rd = getServletContext().getRequestDispatcher("/reports_eco.jsp");
				rd.forward(request, response);
			} 
			catch (Exception e) {
				throw new ServletException(e);
