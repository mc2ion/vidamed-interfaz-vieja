 ...
 */
@WebServlet(description = "servlet to generate reports", urlPatterns = { "/ListBloodBankReportsServlet" })
public class ListBloodBankReportsServlet extends HttpServlet {
     ...
     */
    public ListBloodBankReportsServlet() {
        super();
    }
		...
		if(userE != null && perm){
			try {
				ArrayList<PatientServiceReport> pp = (ArrayList<PatientServiceReport>) CommandExecutor.getInstance().executeDatabaseCommand(
														new command.GetPatientServiceReport("1"));
				request.setAttribute("pp", pp);
				
				RequestDispatcher rd = getServletContext().getRequestDispatcher("/reports_bloodBank.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userE != null && perm ){
			...
			try {
				...
				@SuppressWarnings("unchecked")
				ArrayList<PatientServiceReport> pp = (ArrayList<PatientServiceReport>) CommandExecutor.getInstance().executeDatabaseCommand(
										new command.GetPatientServiceReport("1", adminId, estimationId, identityCard, gender, type, serviceId, dateIni, dateEnd));
				request.setAttribute("pp", pp);
				...
				rd = getServletContext().getRequestDispatcher("/reports_bloodBank.jsp");
				rd.forward(request, response);
			} 
			catch (Exception e) {
				throw new ServletException(e);
