...
/**
 * Servlet implementation class ListRayXReportsServlet
 */
@WebServlet(description = "servlet to generate reports", urlPatterns = { "/ListRayXReportsServlet" })
public class ListRayXReportsServlet extends HttpServlet {
		...
		if(userE != null && perm){
			try {
				ArrayList<PatientServiceReport> pp = (ArrayList<PatientServiceReport>) CommandExecutor.getInstance().executeDatabaseCommand(
														new command.GetPatientServiceReport("4"));
				request.setAttribute("pp", pp);
				
				RequestDispatcher rd = getServletContext().getRequestDispatcher("/reports_rayX.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userE != null && perm ){
			...
			try {
				...
				@SuppressWarnings("unchecked")
				ArrayList<PatientServiceReport> pp = (ArrayList<PatientServiceReport>) CommandExecutor.getInstance().executeDatabaseCommand(
										new command.GetPatientServiceReport("4", adminId, estimationId, identityCard, gender, type, serviceId, dateIni, dateEnd));
				request.setAttribute("pp", pp);
				...
				rd = getServletContext().getRequestDispatcher("/reports_rayX.jsp");
				rd.forward(request, response);
			} 
			catch (Exception e) {
				throw new ServletException(e);
