...
public class ListReportsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if (modId.equals("16")){
			response.sendRedirect(request.getContextPath() + "/DetailedInvoicingReportServlet");
		}	
		...
		else if (modId.equals("19")){
			response.sendRedirect(request.getContextPath() + "/PendingAccountsSummaryReportServlet");
		}	
		else if (modId.equals("20")){
			response.sendRedirect(request.getContextPath() + "/PatientRecordsReportServlet");
		}	
		else if (modId.equals("21")){
			response.sendRedirect(request.getContextPath() + "/BilledSuppliesReportServlet");
		}	
		else if (modId.equals("22")){
			response.sendRedirect(request.getContextPath() + "/PatientSuppliesReportServlet");
		}
		else{
			RequestDispatcher rd = getServletContext().getRequestDispatcher("/reports.jsp");
			rd.forward(request, response);
